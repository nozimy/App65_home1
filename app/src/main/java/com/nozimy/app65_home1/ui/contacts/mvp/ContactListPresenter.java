...
public class ContactListPresenter<V extends ContactListContract.View> extends BasePresenter<V> implements ContactListContract.Presenter<V> {
    ...
    public ContactListPresenter(ContactListInteractor contactListInteractor, Settings settings, CompositeDisposable compositeDisposable) {
    ...
    public void showDetails(String id) {
        ...
        if (getMvpView().isDualPane()) {
            if (getMvpView().getContactFragment() == null || !id.equals(getMvpView().getContactFragment().getIdFromArguments())) {
...
//                CommonUtils.zipWithTimer(contactListInteractor.getContacts())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe(__-> getMvpView().showLoadingIndicator())
                .subscribe(contactEntities -> {
