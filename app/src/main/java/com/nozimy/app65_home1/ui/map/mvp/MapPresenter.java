...
//    private void showMarker(boolean clicked){
//        mMap.clear();
//        float zoom = 10;
//        if(clicked){
...
//            );
//            mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(defaultLatLng, zoom));
//        }
//    }
...
//                            showMarker(true);
    ...
    public boolean onMarkerClick(Marker marker) {
        Log.d("onMarkerClick","CLIIIIIIIIIIIIIIIIIIIIIIIK: ");
        if(showAllContacts){
            String id = (String) marker.getTag();
            getMvpView().openContactDetails(id);
            Log.d("onMarkerClick","CLIIIIIIIIIIIIIIIIIIIIIIIK: "+ id);
            return true;
        }
        return false;
    }
    ...
    public void saveContactAddress(MyMapFragment.OnMapFragmentInteractionListener listener) {
        if(contactId != null && contactLatLng != null){
            mapInteractor.updateContactAddress(contactId, contactLatLng.latitude, contactLatLng.longitude, contactAddress)
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(() -> {
    ...
    void showMarker2(LatLng contactLatLng, String contactName, String contactAddress, boolean clicked){
        mMap.clear();
        float zoom = 10;
        if(clicked){
            zoom = mMap.getCameraPosition().zoom;
